...
import { closeSync, mkdirSync, openSync, readFileSync, statSync, writeFileSync } from 'fs';
...
function makeDirPath(path$$1, options) {
  ...
  try {
    ...
    if (err.code === 'ENOENT') {
      var target = dirname(path$$1);
      return target !== path$$1 && makeDirPath(target, options) && mkdirSync(path$$1, options.mode);
    }
...
function readDatabase(instance) {
  ...
  try {
    ...
    var file = readFileSync(filename);
